...
const getfetchdata=async()=>{
    try{
    const data=await axios.get("http://localhost:8030/_admin")
...
const handledelete= async(id)=>{
    const data=await axios.delete("http://localhost:8030/delete_admin/"+id)
    if(data.data.success){
        getfetchdata()
                     ...
                     showcustomer.map((e1)=>{
                      return(
                          <tr> 
                            ...
                            <td className='dback'>
                              <a href={`/updateregitser/${e1._id}`}>Edit Details</a>
